...
export async function onRequestPost(context) {
  try {
    ...
    const allowed = new Set(['https://lawnoah.com', 'https://www.lawnoah.com']);
    if (origin && !allowed.has(origin)) return json({ error: 'Invalid origin' }, 403);

    const type = request.headers.get('content-type') || '';
    if (!type.includes('application/json')) return json({ error: 'Unsupported content type' }, 415);

    const data = await request.json();
    if (clean(data.website, 200)) return json({ ok: true }); // honeypot: quietly accept bot submission

    const startedAt = Number(data.startedAt || 0);
    const elapsed = Date.now() - startedAt;
    if (!startedAt || elapsed < 2000 || elapsed > 2 * 60 * 60 * 1000) return json({ error: 'Invalid form session' }, 400);
    ...
    if (!name || !validEmail(email) || !matter || message.length < 10) return json({ error: 'Invalid required fields' }, 400);
    if (data.consentPrivacy !== true || data.consentTransfer !== true) return json({ error: 'Consent required' }, 400);
    ...
    const lines = lang === 'en'
      ? [
          `Name: ${name}`, `Email: ${email}`, `Matter: ${matter}`,
          organization ? `Company / Organization: ${organization}` : '',
          position ? `Position: ${position}` : '', country ? `Country: ${country}` : '',
          '', 'Inquiry:', message
        ]
      : [`이름: ${name}`, `회신 이메일: ${email}`, `상담 분야: ${matter}`, '', '문의 내용:', message];

    const text = lines.filter((line, i) => line !== '' || (i > 0 && lines[i-1] !== '')).join('\n');
    const resendResponse = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { Authorization: `Bearer ${resendApiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ from: 'Law Office Noah <info@lawnoah.com>', to: 'info@lawnoah.com', reply_to: email, subject, text })
    });
    ...
    console.error('Contact form server error');
